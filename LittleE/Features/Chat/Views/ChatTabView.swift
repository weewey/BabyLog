...
struct ChatTabView: View {
    ...
    /// Off by default — Qwen3.5 4B is device-only, ~2.5 GB to
    /// download, and the llama.cpp path has only had a single smoke
    /// pass on hardware. Users opt in via Settings → Chat.
    ...
    var body: some View {
        NavigationStack {
            ...
            .toolbar { backendMenu }
            .onAppear {
                if !qwenEnabled, viewModel.selectedBackend == .qwen {
                    viewModel.switchBackend(.claude)
                }
            }
            .onChange(of: qwenEnabled) { _, enabled in
                if !enabled, viewModel.selectedBackend == .qwen {
                    viewModel.switchBackend(.claude)
                }
            }
        ...
        .onAppear {
            // Apple FM is temporarily hidden from the picker. If a prior
            // launch persisted `.apple` as the selection, migrate to Claude
            // so the user isn't stuck on a backend they can't switch off.
    ...
    private var appleToolHint: some View {
        HStack(spacing: 8) {
            Image(systemName: "info.circle")
                .foregroundStyle(.secondary)
            Text("Apple on-device chat can't run tools yet. Switch to Claude or Gemma to log feeds by voice.")
                .font(.footnote)
                .foregroundStyle(.secondary)
    ...
    private var backendMenu: some ToolbarContent {
        ToolbarItem(placement: .topBarTrailing) {
            ...
            .accessibilityHint("Tap to switch between Claude and Gemma backends")
            .accessibilityIdentifier("chatBackendMenu")
        }
    }

    /// Backends currently shown in the picker. Apple FM is temporarily
    /// hidden — it can't dispatch tools yet and the chat tab is tool-first.
    /// Qwen is gated behind a Settings toggle since it's device-only and
    /// the llama.cpp path is still experimental.
    private var visibleBackends: [ChatBackend] {
        var list: [ChatBackend] = [.claude, .gemma]
        if qwenEnabled { list.append(.qwen) }
        return list
    }
    ...
    private func errorCopy(_ error: ChatViewModel.Error) -> String {
        ...
        case .attachmentNotSupported:
            return "This chat backend doesn't support images. Switch to Claude to send the photo."
        }
    }
}
...
private struct EmptyChatView: View {
    ...
    private var babyName: String { profile?.name ?? "Ethan" }
...
private func makeToolCallPreviewVM() -> ChatViewModel {
    ...
    seeded.append(ChatMessage(role: .user, text: "Ethan had 120ml from the bottle"))
