...
**Last updated:** 2026-06-07 (Apple FM selectable in chat picker, with native tool-calling)
...
- **Gemma model loads on-demand, not at launch** (2026-06-07): removed the eager `Gemma4MLXChatSession.startWarmUp()` from `BabyLogApp.init` (its comment "Gemma is the only chat backend" was stale). Now that the chat defaults to Apple FM, warming Gemma at every launch meant users who never pick Gemma still paid the ~1.5 GB download + load. Gemma now loads lazily on the first `stream()` that actually uses it (`ensureContainer`, surfaced as a `.modelLoading` progress bar). Removed the now-unused `startWarmUp`/`warmUp` statics; kept `inFlightTask`/`setInFlight`/`currentInFlight` (still used by `stream()` to serialize MLX generations — two concurrent inferences crash MLX). Settings still has an explicit on-demand "Download Gemma 4" row.
- **Default chat to Apple FM (MLX lock-crash avoidance)** (2026-06-07): TestFlight build 28 still `SIGABRT`'d on `com.Metal.CompletionQueueDispatch` with `isLocked=1` (`BabyLog-2026-06-07-174536.ips`) — the documented MLX/Gemma lock-during-generation crash (device locked mid-reply → iOS 26 revokes GPU → in-flight Metal command buffer completes-with-error → MLX rethrows uncatchably off any Swift stack). The build 22–24 mitigations reduce but **cannot** eliminate a deliberate power-button lock; it's not fixable app-side for in-process MLX. Only complete fix: don't run MLX by default. New `AppleFMAvailability.isAvailable` (gates on `SystemLanguageModel.default.availability`) + `ChatBackendDefaulting.resolveDefault` now default the chat to crash-immune **Apple FM** whenever Apple Intelligence is available, and perform a **one-time migration** of any stored Gemma/Qwen selection onto Apple FM (respects a later deliberate switch back). Wired in `RootTabView.makeChatViewModel`; forced off under `--ui-testing` so fake-chat tests stay on Gemma. Gemma stays selectable in the picker. 4 new `ChatBackendDefaultingTests`. See [[mlx-ios26-gpu-revoke-crash]], [[apple-fm-immune-to-gpu-crash]].
- **Pediatrician-visit summary (F2)** (2026-06-07): new "Prep for visit" flow on the Appointments tab. **V1** — `VisitSummary` + `VisitSummaryBuilder` in Core (pure, 8 Linux tests) assemble feeds/diapers/growth/pumping/milestones over a window starting at the most recent past appointment (else last 14 days); `VisitSummary.plainText` is the share payload. All figures deterministic — no model. `VisitSummaryViewModel` fetches every repo + runs the builder; `VisitSummaryView` (structured List + `ShareLink`) is presented from `AppointmentTabView`'s toolbar, wired in `RootTabView`. **V2** — `VisitSummary.narrationPrompt` + on-device Apple FM narration (best-effort: streams a warm paragraph above the card, silently falls back to the structured summary when the model is unavailable/fails; prompt forbids inventing numbers or giving medical advice). 5 VM tests (load success/failure, narration success/failure/disabled). Grounded in PM review; deliberately reuses the LLM + analytics rather than adding infra.
- **Apple FM: reinstate edit tools + soften late generation errors** (2026-06-07): (1) Reverted the `update*`/`delete*` tool curation — Apple FM is given the full 24-tool set again so it can edit/remove entries by voice, not just create. Eval confirms no accuracy loss (86.7% tool / 85% param with all 24 vs 85/80 curated) and 0 errors. (2) Fixed a user-reported `LanguageModelSession.GenerationError` (`com.apple.token-generation Code=10`) that fired **mid-reply after the tool already ran** and a usable summary had streamed, surfacing a scary "Chat error" modal even though the work succeeded. `ChatViewModel.drain` now tracks `producedOutput` (a non-empty token or a completed tool result) and, for internally-executing backends (Apple FM), seals the turn as a normal completion when a late error arrives after usable output — only hard-failing when nothing usable was produced. Multi-turn context safety still comes from `renderTranscript`'s `maxTranscriptMessages` trim + the 512-token output cap. +2 `ChatViewModelTests` (late-error seals, early-error still surfaces).
- **Apple FM speed + context-window fixes** (2026-06-07): on-device Apple FM felt slow and threw `.exceededContextWindowSize` because all ~24 tool schemas + the full transcript were serialized into the ~4k-token window every turn. Fixes: (a) `AppleFMChatSession.curatedTools` drops `update*`/`delete*` tools (24→12) — the 3B model can't drive edit flows anyway (needs a list-to-get-id step) and the eval never expects them, so cutting them halves the tool payload with no accuracy loss; (b) `renderTranscript` now keeps only the last `maxTranscriptMessages` (12) so long chats don't grow the prompt unbounded; (c) `session.prewarm()` + `GenerationOptions(maximumResponseTokens: 512)` for faster first token and bounded output. **Dead end (documented in `AppleFMToolAdapter`):** setting `includesSchemaInInstructions = false` to save tokens makes Apple FM stop calling tools entirely — it hallucinates success ("I've logged it") without invoking anything, silently dropping data (eval: tool accuracy 88%→15%). Reverted. Eval after fix: 85% tool / 80% param with **0/60 context errors** (vs up to 1/60 with 24 tools). Edits remain available on Gemma + the per-domain UI.
- **Apple FM selectable in chat picker, with native tool-calling** (2026-06-07): Apple Foundation Models (`.apple`) is now a real option in the Chat backend menu alongside Gemma (`ChatTabView.visibleBackends` = `[.apple, .gemma]`; removed the `.onAppear` force-switch-to-Gemma and the "can't run tools yet" hint). Apple FM now runs the logging tools: new `AppleFMToolAdapter` (a `FoundationModels.Tool` with `Arguments = GeneratedContent`) wraps each `ChatTool`, `AppleFMToolMapping` builds a `GenerationSchema` from `ToolInputSchema` (via `DynamicGenerationSchema`; `date-time`→String, `enumValues`→`anyOf`) and decodes `GeneratedContent.jsonString` → `ToolArguments`. Apple's `LanguageModelSession` owns the tool loop (executes adapters in-context, schemas sent once — important for the ~4k token window vs ~25 tools), so `AppleFMChatSession` sets a new `ChatSession.executesToolsInternally` (default `false`) flag: `ChatViewModel.drain` records the backend-emitted `.toolCall`/`.toolResult` deltas for the invocation card **without** re-executing or re-looping. `AppleFMChatSession` rewritten onto a unified `AppleFMEvent` (`.text`/`.toolCall`/`.toolResult`) seam so tool events and cumulative text snapshots arrive in generation order; `renderTranscript` now carries prior tool calls/results forward for multi-turn. `LiveChatSessionFactory.appleInstructions` enriched (tool etiquette, local-time rule, today's date, no-id rule). Apple FM stays crash-immune (out-of-process, throwing). New tests: ChatViewModel internal-execution path (2), AppleFMChatSession tool passthrough (3), AppleFMToolMapping/adapter against real FoundationModels (3), Core default-flag (1); E2E backend-picker test switches to Apple + persists. Known v1 limitation: under single-turn internal execution the assistant summary bubble renders above the tool cards (Gemma shows cards above text); 25 tool schemas may pressure the 4k context (surfaces as a catchable `.exceededContextWindowSize`, not a crash). **Eval-tuned** via a new real on-device harness (`scripts/eval/apple_fm_eval.swift` — drives the actual FM model natively on macOS 26, unlike `run_eval.py` which proxies on-device prompts through Claude): on the 60-row tool dataset the original `appleInstructions` scored 76.7% tool / 75% param; tightening it to "act with sensible defaults, never ask clarifying questions, log each action the parent mentions" (dropping the harmful "call a tool only once per request" line) lifted it to ~88% tool / ~84% param (mean of 3 runs). Residual weakness: terse one-word inputs and "correct the last entry" edit flows (model skips the list-to-get-id step).
